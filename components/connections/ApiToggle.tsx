...
export default defineComponent({
  ...
  setup: (props, { emit }) => {
    ...
    const grpcMessage = computed(() =>
      !props.node.settings.grpc ? "GRPC is disabled" : ""
    );
    const restMessage = computed(() =>
      !props.node.settings.rest ? "REST is disabled" : ""
    );

    const combinedMessage = computed(() =>
      [grpcMessage.value, restMessage.value].filter((e) => !!e)
    );
    ...
    return () => (
      <v-radio-group onChange={update} value={props.value}>
        <v-radio
          value="GRPC"
          key="GRPC"

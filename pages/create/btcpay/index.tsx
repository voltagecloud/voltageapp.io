...
export default defineComponent({
  ...
  // @ts-ignore
  setup: (_, { refs, root }) => {
    const mainnetNodes = computed(() => nodeStore.nodeNetwork(Network.mainnet));

    //make sure the user has btcpay servers
    const { dispatch, data, loading: serverLoading } = useFetch<{
      btcpayservers: any[];
    }>("/btcpayserver");
    dispatch({ method: "GET" });
    ...
    const canCreate = computed(() => {
      ...
      return (
        data.value.btcpayservers.length === 0 &&
        (userData.value.available_btcpayservers ||
    ...
    async function validateForm() {
      ...
      if (!formValid) return;
      const keysValid =
        (state.createKeys === "generate" && state.walletPayload) ||
        state.createKeys === "useown";
    ...
    async function createBtcPay() {
      ...
      const { macaroonHex } = macaroonStore.macaroonState({
        nodeId,
        type: "btcpayserver",
      });
          ...
          <v-col cols="12" xl="8">
            <v-container>
              <v-card color="info" key="0">
                <v-card-title>
                  <div class="mx-auto" style="word-break: normal;">
                    Create BTCPay Server Store
                  </div>
                </v-card-title>
                <v-card-actions>
                  <v-form style="width: 100%;" ref="form" lazy-validation>
                      ...
                      <v-row>
                        <v-col cols="12">
                          <v-text-field
                            ...
                            rules={[
                              (val: string) =>
                                !!val || "Your store must have a name",
                            ]}
                          />
                        </v-col>
                        <v-col cols="12">
                        ...
                        <v-container>
                          <v-row>
                            <v-col cols="12">
                              <div class="ml-3">
                                Create a Bitcoin wallet for your store:
                              </div>
                            </v-col>
                            <v-col cols="12" sm="6">
                              <v-btn
                          ...
                          <v-dialog
                            ...
                            onInput={() => (state.createKeys = null)}
                          >
                            <v-card>
                              <v-card-actions>
                                <v-btn
                                  icon
                                  onClick={() => (state.createKeys = null)}
                                >
                              ...
                              <keep-alive>
                                <CreateBTCWallet
                                  onFinalize={(payload: WalletPayload) =>
                                    (state.walletPayload = payload)
                                  }
                                />
                              </keep-alive>
                            </v-card>
                    ...
                    <v-container>
                      ...
                      <div class="error--text">
                        {!canCreate.value && !loading.value
                          ? `You don't have any available btcpay server accounts`
                          : ""}
                      </div>
                    </v-container>
                  </v-form>
              ...
              >
                <v-card class="text-center">
                  <v-container>
                    ...
                    <div class="text-h6">
                      Your BTCPay Server Account has successfully been created.
                    </div>
                    <v-row class="my-4">
                      ...
                      <v-col cols="12">
                        Username: <copy-pill text={username.value} />
                      </v-col>
                      <v-col cols="12">
                        Password: <copy-pill text={btcpayState.password} />
                      </v-col>
                    </v-row>
                    <div class="my-4 font-italic">
                      Please change the default password as soon as you login
                    </div>
                    <v-btn color="highlight" dark large onClick={finish}>
                      Login to Account
                    </v-btn>
                  </v-container>
                </v-card>
